...
dy = -10
for i in range(0,6):
    phot1 = Photon(Vector(600,dy+3*i),Vector(-1,0),15)
    photons.append(phot1)
for i in range(0,10):
...
def draw():
    ...
    for a in atoms:
        for p in photons:
            if a.interaction(p) and frame_count > 10 + a.inteaction_frame:
                if a.n0_occ == 1:
                    photons.remove(p)
                    a.excite()
                elif a.n1_occ == 1:
    ...
    with push_matrix():
        with push_style():
            ...
            Text("3. the photon will bounce around and create more photons by stimulated emission",(200,-300),size=40, font = "i")
    saver()
